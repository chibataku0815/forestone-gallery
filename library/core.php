...
if ( ! function_exists( 'cb_menu_icons' ) ) {
    function cb_menu_icons( $cb_location = NULL ){
        ...
        $cb_menu_output = $cb_s_tooltip = $cb_menu_tw_title = $cb_menu_fb_title = $cb_menu_go_title = $cb_menu_pi_title = $cb_menu_in_title = $cb_menu_sc_title = $cb_menu_yt_title = $cb_menu_tum_title = NULL;
            ...
            if ( $cb_menu_extras_title == 'cb-menu-extras-title-tooltip' ) {
                ...
                $cb_menu_yt_title = 'class="cb-tip-bot" data-cb-tip="YouTube"';
                $cb_menu_sc_title = 'class="cb-tip-bot" data-cb-tip="Snapchat"';
                $cb_menu_tum_title = 'class="cb-tip-bot" data-cb-tip="Tumblr"';
            ...
            $cb_menu_output .= ot_get_option( 'cb_nav_yt', 'off' ) == 'on' ? '<li class="cb-menu-icons-extra cb-social-icons-extra"><a href="' . esc_url( 'http://www.youtube.com/' . ot_get_option( 'cb_nav_yt_url', NULL ) ) . '" ' . $cb_menu_yt_title . ' target="_blank"><i class="fa fa-youtube-play" aria-hidden="true"></i></a></li>' : NULL;
            $cb_menu_output .= ot_get_option( 'cb_nav_sc', 'off' ) == 'on' ? '<li class="cb-menu-icons-extra cb-social-icons-extra"><a href="' . esc_url( 'http://www.snapchat.com/add/' . ot_get_option( 'cb_nav_sc_url', NULL ) ) . '" ' . $cb_menu_sc_title . ' target="_blank"><i class="fa fa-snapchat-ghost" aria-hidden="true"></i></a></li>' : NULL;
            $cb_menu_output .= ot_get_option( 'cb_nav_tum', 'off' ) == 'on' ? '<li class="cb-menu-icons-extra cb-social-icons-extra"><a href="' . esc_url( 'http://' . ot_get_option( 'cb_nav_tum_url', NULL ) . '.tumblr.com' ) . '" ' . $cb_menu_tum_title . ' target="_blank"><i class="fa fa-tumblr" aria-hidden="true"></i></a></li>' : NULL;
    ...
    function cb_get_trend_count() {
        ...
        if ( function_exists( 'stats_get_csv' ) ) {
            ...
            foreach ( $cb_qry as $cb_post ) {
                ...
                $cb_img = cb_get_thumbnail( '600', '400', $cb_post_id );
...
if ( ! function_exists( 'cb_s_a' ) ) {
    function cb_s_a() {
        ...
        $cb_s = ( isset( $_GET['cbi'] ) ) ? $_GET['cbi'] : NULL;
        $cb_cpt_output = cb_get_custom_post_types();
        if ( ot_get_option('cb_show_pages_search', 'off' ) != 'off' ) {
            $cb_cpt_output[] = 'page';
        }
        $cb_args = array( 's' => esc_html( $cb_s ), 'ignore_sticky_posts' => 1, 'post_status' => 'publish', 'post_type' => $cb_cpt_output );
...
if ( ! function_exists( 'cb_ajax_post_search' ) ) {
    function cb_ajax_post_search() {
        ...
        $cb_featured_qry = array( 's' => $cb_current_string, 'posts_per_page' => -1,  'post_status' => 'publish' );
...
if ( ! function_exists( 'cb_byline' ) ) {
    function cb_byline( $cb_post_id, $cb_cat = NULL, $cb_is_post = NULL, $cb_read_time = NULL, $cb_views_on = NULL ) {
        echo cb_get_byline( $cb_post_id, $cb_cat, $cb_is_post, $cb_read_time, $cb_views_on );
    }
}
...
if ( ! function_exists( 'cb_get_byline' ) ) {
    function cb_get_byline( $cb_post_id, $cb_cat = NULL, $cb_is_post = NULL, $cb_read_time = NULL, $cb_views_on = NULL, $cb_override = NULL, $args = array() ) {
        ...
        $updated_class = empty( $args['schema'] ) ? '' : 'updated';
        $author_class = empty( $args['schema'] ) ? '' : ' vcard author';
        ...
        if ( $cb_byline_author != 'off' ) {
            ...
            if ( function_exists( 'coauthors_posts_links' ) ) {
               ...
               $cb_author =  apply_filters( 'cb_byline_author', '<span class="cb-author' . esc_attr( $author_class ) . '"><span class="fn"><a href="' . esc_url( get_author_posts_url( get_post_field( 'post_author', $cb_post_id ) ) ) . '">' . $cb_author_avatar . get_the_author_meta( 'display_name', get_post_field( 'post_author', $cb_post_id ) ) . '</a></span></span>', $cb_post_id, $cb_author_avatar );
            }
        ...
        if ( $cb_byline_date != 'off' ) {
            $cb_date = apply_filters( 'cb_byline_date', '<span class="cb-date"><time class=" ' . esc_attr( $updated_class ) . '" datetime="' . get_the_time('Y-m-d', $cb_post_id) . '">' . date_i18n( get_option('date_format'), strtotime(get_the_time("Y-m-d", $cb_post_id )) ) . '</time></span>', $cb_post_id );
        }
...
if ( ! function_exists( 'cb_get_review_byline' ) ) {
    function cb_get_review_byline( $cb_post_id ) {
        ...
        $cb_byline_date = ot_get_option('cb_byline_date', 'on');
        $cb_review_final_score = $cb_category_color = $lets_review_onoff = NULL;
        ...
        if ( class_exists( 'Lets_Review_API' ) ) {
            ...
            $lets_review_onoff = $lets_review_api->lets_review_get_onoff( $cb_post_id );
        }
        if ( $cb_review_checkbox == 'on' || $cb_review_checkbox == '1' || $lets_review_onoff == true ) {
            ...
            if ( class_exists( 'Lets_Review_API' ) && $lets_review_onoff == true ) {
                $cb_final_score = get_post_meta( $cb_post_id, '_lets_review_final_score', true );
                $cb_score_display_type = get_post_meta( $cb_post_id, '_lets_review_format', true );
                switch ( $cb_score_display_type ) {
                    case 1:
                        $cb_score_display_type = 'percentage';
                        break;
                     case 2:
                        $cb_score_display_type = 'points';
                        $cb_final_score = $cb_final_score * 10;
                        break;

                    default:
                        $cb_score_display_type = 'stars';
                        $cb_final_score = $cb_final_score * 20;
                        break;
                }
...
if ( ! function_exists( 'cb_pagination_offset' ) ) {
    function cb_pagination_offset($found_posts, $query) {
        ...
        return ( intval( $found_posts ) - intval( $cb_grid_size ) );
    }
}
...
?>
                <li <?php post_class( 'cb-style-1 clearfix cb-no-' . $i ); ?>>
                    <div class="cb-mask"><?php cb_thumbnail( '600', '400', $post->ID );?></div>
                    <div class="cb-meta">
...
if ( ! function_exists( 'cb_get_featured_image_style' ) ) {
    function cb_get_featured_image_style( $cb_featured_image_style, $post, $cb_page = NULL ) {
        ...
        if ( $cb_page == NULL ) {
            ...
            $cb_title .= '<div class="cb-entry-header hentry cb-meta clearfix">';
            $cb_title .= '<h1 class="entry-title cb-entry-title entry-title cb-title">' . get_the_title() . '</h1>';
            ...
            if ( ( ( $cb_featured_image_style != 'standard' ) && ( $cb_featured_image_style != 'standard-uncrop' ) && ( $cb_featured_image_style != 'off' ) && ( $cb_featured_image_style != NULL ) ) || ( $cb_post_format == 'gallery' ) ) {
                $cb_title .= cb_get_byline( $cb_post_id, true, true, true, true, NULL, array( 'schema' => true ) );
            } else {
                $cb_title .= cb_get_byline( $cb_post_id, NULL, true, NULL, true, NULL, array( 'schema' => true ) );
            }
...
if ( ! function_exists( 'cb_about_author' ) ) {
    function cb_about_author( $post, $cb_author_id_sc = NULL ) {
        ...
        $cb_author_output .= '<div class="cb-title cb-font-header"><a href="' . get_author_posts_url( $cb_author_id ) . '"><span>' . $cb_author_name . '</span></a>';
        $cb_author_output .= '</div>';
        if ( $cb_author_position != NULL ) { $cb_author_output .= '<div class="cb-author-position cb-font-header">' . $cb_author_position . '</div>';}

        if ( $cb_author_desc != NULL ) { $cb_author_output .= '<p class="cb-author-bio">' . $cb_author_desc . '</p>'; }

        if ( ( $cb_author_email != NULL ) || ( $cb_author_www != NULL ) || ( $cb_author_tw != NULL ) || ( $cb_author_instagram != NULL ) || ( $cb_author_googleplus != NULL ) || ( $cb_author_facebook != NULL ) ) { $cb_author_output .= '<div class="cb-author-page-contact">'; }
        if ( $cb_author_email != NULL ) { $cb_author_output .= '<a href="mailto:' . sanitize_email( $cb_author_email ) . '" class="cb-contact-icon cb-tip-bot" data-cb-tip="' . __('Email', 'cubell') . '"><i class="fa fa-envelope-o"></i></a>'; }
        if ( $cb_author_www != NULL ) { $cb_author_output .= ' <a href="' . esc_url( $cb_author_www ) . '" target="_blank" class="cb-contact-icon cb-tip-bot" data-cb-tip="'. __('Website', 'cubell') . '"><i class="fa fa-link"></i></a> '; }
        if ( $cb_author_tw != NULL ) { $cb_author_output .= ' <a href="//www.twitter.com/' . $cb_author_tw . '" target="_blank" class="cb-contact-icon cb-tip-bot" data-cb-tip="Twitter"><i class="fa fa-twitter"></i></a>'; }
        if ( $cb_author_googleplus != NULL ) { $cb_author_output .= ' <a href="' . esc_url( $cb_author_googleplus ) . '" target="_top" class="cb-contact-icon cb-tip-bot" data-cb-tip="Google+"><i class="fa fa-google-plus"></i></a>'; }
        if ( $cb_author_facebook != NULL ) { $cb_author_output .= ' <a href="' . esc_url( $cb_author_facebook ) . '" target="_top" class="cb-contact-icon cb-tip-bot" data-cb-tip="Facebook"><i class="fa fa-facebook"></i></a>'; }
        if ( $cb_author_instagram != NULL ) { $cb_author_output .= ' <a href="' . esc_url( $cb_author_instagram ) . '" target="_top" class="cb-contact-icon cb-tip-bot" data-cb-tip="Instagram"><i class="fa fa-instagram"></i></a>'; }
        if ( ( $cb_author_email != NULL ) || ( $cb_author_www != NULL ) || ( $cb_author_instagram != NULL ) || ( $cb_author_googleplus != NULL ) || ( $cb_author_tw != NULL ) || ( $cb_author_facebook != NULL ) ) {$cb_author_output .= '</div>';}
        $cb_author_output .= '</div>';
        ...
        if ( $cb_post_format_type == 'gallery' ) {
            ...
            if ( $cb_post_gallery != NULL ) {
                ...
                $cb_output = '<div id="cb-featured-image" class="cb-fis cb-fis-big cb-fis-fs cb-fis-block-background cb-gallery-post-wrap clearfix">';
                 ...
                 $cb_output .= '<div class="cb-entry-header hentry cb-meta">';
                    $cb_output .= '<h1 class="entry-title cb-entry-title entry-title cb-title">' . get_the_title() . '</h1>' . cb_get_byline( $cb_post_id, '', '', '', '', '',  array( 'schema' => true ) );
                    $cb_output .= '</div>';
...
HENTRY CLASS
*********************/
if ( ! function_exists( 'cb_hentry_class' ) ) {
    function cb_hentry_class( $classes ) {
        $classes = array_diff( $classes, array( 'hentry' ) );
        return $classes;
    }
}
add_filter( 'post_class', 'cb_hentry_class' );
